...
我们要研究的方程是
\begin{align*}
   & \mathrm{i}u_{t} + \alpha_{1}(t)u_{xx} + \alpha_2(t)u^{2}u^*(-x,t) + \mathrm{i}\left[\alpha_3(t)u_{xxx} + \alpha_{4}(t)[u^{2}u^*(-x,t)]_{x}  \right. \\ \notag
  & \left. + \alpha_{5}(t)[uu^*(-x,t)]_{x}u + \alpha_{6}(t)u + \alpha_{7}(t)u_{x} \right] = 0  \notag
\end{align*}

\begin{equation}
  au_t + \frac{3}{2}b(t)uu^*(-x,t)u_x - \frac{1}{2}b(t)u^2u^*_x(-x,t)+\frac{b(t)}{6a^2}u_{xxx} = 0
\end{equation}
...
\begin{align}
  ...
  & V_{3} = \begin{pmatrix}
            ...
            \end{pmatrix} \\
    & U = \begin{pmatrix}
             -\mathrm{ia\lambda} &  au^*[-x,t] &  au[x,t] \\
              -au[x,t] & \mathrm{ia\lambda} & 0 \\
              -au^*[-x,t] & 0 & \mathrm{ia\lambda}
              \end{pmatrix} \\
\end{align}

\begin{equation*}
  V = b(t)\begin{pmatrix}
             -\frac{2i\lambda^3}{3}+2\lambda A_1 & \lambda A_2+A_4+\frac{2}{3}\lambda^2u^*[-x,t] &  \frac{2}{3}\lambda^2u -\lambda A^*_2[-x,t]+A_4^*[-x,t] \\
             -\frac{2}{3}\lambda^2u -\lambda A_2^*[-x,t]-A_4^*[-x,t]  & \frac{2i\lambda^3}{3}+A_5 +\lambda A_1^*[-x,t] & -\lambda A_3^*[-x,t] \\
             \lambda A_2 - A_4 - \frac{2}{3}\lambda^2u^*[-x,t] & \lambda A_3 & \frac{2i\lambda^3}{3}+\lambda A_1^*[-x,t]-A_5^*[-x,t]
              \end{pmatrix} \\
\end{equation*}
...
\begin{align}
  ...
  & A_{1} =  \frac{\mathrm{i}}{3}uu^{*}[-x,t] \\
  & A_{2} = -\frac{\mathrm{i}u^{*}_{x}[-x,t]}{3a}   \\
  & A_{3} = -\frac{\mathrm{i}}{3}(u^{*}[-x,t])^{2}  \\
  & A_{4} = \frac{-4a^2uu^*[-x,t]^2-u^*_{xx}[-x,t]}{6a^2} \\
  & A_{5} = -\frac{u^*[-x,t]u_x}{6a} - \frac{uu^*_x[-x,t]}{6a}
\end{align}
