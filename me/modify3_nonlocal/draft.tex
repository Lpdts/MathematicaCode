...
我们要研究的方程是
\begin{equation}
\mathrm{i}u_{t} + \alpha_{1}(t)u_{xx} + \alpha_2(t)u^{2}u^*(-x,t) + \mathrm{i}[\alpha_3(t)u_{xxx} + \alpha_{4}(t)[u^{2}u^*(-x,t)]_{x} + \alpha_{5}(t)[uu^*(-x,t)]_{x}u + \alpha_{6}(t)u] = 0
\end{equation}
其中方程(1)需要满足以下的约束条件
\begin{align}
...
其中$k(x,t)=k_{3}(t)e^{k_{1}(t)x + \mathrm{i}k_{2}(t)}$，并满足相容条件
\begin{equation}
  U_{t} - V_{x} + UV - VU = 0
\end{equation}
经计算可以得到以下的结果
\begin{align}
  & \beta = \pm \sqrt{3}aC_{2} \\
  & a(t) = a \\
  & b(t) = 6a^{3}\alpha_{3}(t) \\
  & k_{3}(t) = Ce^{\int \alpha_{6}(t)dt}   \\
  & k = k_{3}(t)\mathrm{Exp}\left[\left(\beta x + (2\mathrm{i}\beta^{3} + 6a^{3}C_{6})\int \alpha_{3}(t)dt \right)\right] \\
  & A_{1} =  -\mathrm{i}C_2^2 + \frac{\mathrm{i}}{3}k_3(t)^2uu^*(-x,t) \\
  & A_{2} = \frac{\mathrm{i}\beta u^*(-x,t)-\mathrm{i}u^*_x(-x,t)}{3a}  \\
  & A_{3} = -\frac{\mathrm{i}}{3}u^{*}(-x,t)^{2}  \\
  & A_{4} = C_2^2u^*(-x,t)-\frac{2}{3}k_3(t)^2uu^*(-x,t)^2 - \frac{-2\beta u^*_x(-x,t)+u^*_{xx}(-x,t)}{6a^2} \\
  & A_{5} = \mathrm{i}C_6 - \frac{k_3(t)^2(u^*(-x,t)u_x(x,t)+uu^*_x(-x,t))}{6a}
\end{align}
...
\begin{align}
  & f(t) = d_{1}\mathrm{Exp}\left[(-6\mathrm{i}a^{3}C_{6} - 8\mathrm{i}a^{3}\eta^{3} - 18\mathrm{i}a^{3}\eta C_{2}^2)\int \alpha_{3}(t)dt\right] \\
  & g(t) = d_{2}
\end{align}
从而可以得到
\begin{align}
  & \Gamma_{1} = d_{1}\mathrm{Exp}\left[2a\eta x - (6\mathrm{i}a^{3}C_{6} + 8\mathrm{i}a^{3}\eta^{3} + 18\mathrm{i}a^{3}\eta C_{2}^2)\int \alpha_{3}(t)dt\right] \\
  & \Gamma_{2} = d_{2}
\end{align}
其中$d_{1}, d_{2}$是复常数，带入原式可得出$d_{2} = 1$, 将 $(49), (50)$ 代入到 $(42)$ 式可得到单孤子解
\begin{align}
  u(x,t) = \frac{4\eta d_{1}\mathrm{Exp}\Big[(2a\eta+\beta)x + \mathrm{i}(8a^{3}\eta^{3} + 2\mathrm{i}\beta^{3} + 18a^{3}\eta C_{2}+12a^3C_6)\int \alpha_{3}(t)dt - \int \alpha_{6}(t)dt\Big]}{C d_{1}^{2}\mathrm{Exp}[4a\eta x] + 2C \mathrm{Exp}\Big[(16\mathrm{i}a^{3}\eta^{3}+36\mathrm{i}a^{3}\eta C_{2}^2+12\mathrm{i}a^3C_6)\int \alpha_{3}(t)dt\Big]}
\end{align}
...
\begin{align}
  ...
  & \left. - kA_{4} - \frac{2}{3}\lambda^2 ku^{*})T_{1}^{2} + (-\frac{2}{3}\lambda^{2}k^{*}u-\lambda k^{*}A_{2}^{*}-k^{*}A_{4}^{*})T_{1}T_{2} \right]b(t) \\
